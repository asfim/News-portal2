...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark mb-1">Edit Advertisement</h2>
            <p class="text-secondary mb-0">Modify campaign details, dates, or assets code.</p>
        </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-8">
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control py-3" id="title" name="title" value="{{ old('title', $advertisement->title) }}" placeholder="e.g. Header Leaderboard Google Ads" required autofocus>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                ...
                                <select class="form-select py-3" id="placement_key" name="placement_key" required>
                                    <option value="header_banner" {{ old('placement_key', $advertisement->placement_key) == 'header_banner' ? 'selected' : '' }}>Header Banner (728x90)</option>
                                    <option value="sidebar_top" {{ old('placement_key', $advertisement->placement_key) == 'sidebar_top' ? 'selected' : '' }}>Sidebar Top (300x250)</option>
                                    <option value="sidebar_bottom" {{ old('placement_key', $advertisement->placement_key) == 'sidebar_bottom' ? 'selected' : '' }}>Sidebar Bottom (300x600)</option>
                                    <option value="inside_content" {{ old('placement_key', $advertisement->placement_key) == 'inside_content' ? 'selected' : '' }}>Inside Content (Fluid)</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select class="form-select py-3" id="type" name="type" required>
                                    <option value="image" {{ old('type', $advertisement->type) == 'image' ? 'selected' : '' }}>Upload Image Banner</option>
                                    <option value="code" {{ old('type', $advertisement->type) == 'code' ? 'selected' : '' }}>Custom Script Code / AdSense</option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="mb-4">
                                ...
                                @if($advertisement->image_path)
                                    <div class="p-2 bg-white rounded-3 border d-inline-block">
                                        <img src="{{ $advertisement->image_path }}" alt="Current Banner" style="max-height: 80px;">
                                    </div>
                                @endif
                            </div>
                            <div class="mb-3">
                                <label for="redirect_url" class="form-label fw-semibold text-secondary">Redirect Destination URL</label>
                                <input type="url" class="form-control py-3" id="redirect_url" name="redirect_url" value="{{ old('redirect_url', $advertisement->redirect_url) }}" placeholder="https://advertiser-website.com">
                            </div>
                        </div>
                        ...
                        <div id="codeFields" class="mb-4 d-none">
                            ...
                            <textarea class="form-control" id="script_code" name="script_code" rows="6" placeholder="Paste Google AdSense tag or HTML embed iframe scripts here...">{{ old('script_code', $advertisement->script_code) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card bg-light bg-opacity-25 border-0 p-4 rounded-3 mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="date" class="form-control py-3" id="start_date" name="start_date" value="{{ old('start_date', $advertisement->start_date ? \Carbon\Carbon::parse($advertisement->start_date)->format('Y-m-d') : '') }}">
                            </div>

                            <div class="mb-4">
                                <label for="end_date" class="form-label fw-semibold text-secondary">End Date</label>
                                <input type="date" class="form-control py-3" id="end_date" name="end_date" value="{{ old('end_date', $advertisement->end_date ? \Carbon\Carbon::parse($advertisement->end_date)->format('Y-m-d') : '') }}">
                            </div>
                            ...
                            <div class="form-check form-switch ps-0">
                                ...
                                <input class="form-check-input ms-0 border-secondary-subtle" style="width: 2.5em; height: 1.25em;" type="checkbox" id="status" name="status" value="1" {{ old('status', $advertisement->status ? '1' : '0') == '1' ? 'checked' : '' }}>
                            </div>
                        </div>
                    </div>
                </div>

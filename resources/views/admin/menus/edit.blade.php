...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark mb-1">Edit Menu Item</h2>
            <p class="text-secondary mb-0">Modify menu positioning, labels, or target slugs.</p>
        </div>
                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control py-3" id="label" name="label" value="{{ old('label', $menu->label) }}" placeholder="e.g. Home, Sports, Contact Us" required autofocus>
                        </div>

                        <div class="mb-4">
                            <label for="value" class="form-label fw-semibold text-secondary">Target Value (Slug / URL)</label>
                            <input type="text" class="form-control py-3" id="value" name="value" value="{{ old('value', $menu->value) }}" placeholder="e.g. /category/sports, /contact" required>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                ...
                                <select class="form-select py-3" id="type" name="type" required>
                                    <option value="custom" {{ old('type', $menu->type) == 'custom' ? 'selected' : '' }}>Custom Link</option>
                                    <option value="category" {{ old('type', $menu->type) == 'category' ? 'selected' : '' }}>Category Page</option>
                                    <option value="subcategory" {{ old('type', $menu->type) == 'subcategory' ? 'selected' : '' }}>Subcategory Page</option>
                                    <option value="page" {{ old('type', $menu->type) == 'page' ? 'selected' : '' }}>Static Page</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label for="sort_order" class="form-label fw-semibold text-secondary">Sorting Display Order</label>
                                <input type="number" class="form-control py-3" id="sort_order" name="sort_order" value="{{ old('sort_order', $menu->sort_order ?? 0) }}" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card bg-light bg-opacity-25 border-0 p-4 rounded-3 mb-4">
                            ...
                            <div class="form-check form-switch ps-0">
                                ...
                                <input class="form-check-input ms-0 border-secondary-subtle" style="width: 2.5em; height: 1.25em;" type="checkbox" id="status" name="status" value="1" {{ old('status', $menu->status ? '1' : '0') == '1' ? 'checked' : '' }}>
                            </div>
                        </div>
                    </div>
                </div>

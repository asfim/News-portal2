...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark mb-1">Edit Subcategory</h2>
            <p class="text-secondary mb-0">Modify settings or re-parent subcategory.</p>
        </div>
                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        <div class="mb-4">
                            ...
                            <select class="form-select py-3" id="parent_id" name="parent_id" required>
                                ...
                                @foreach($parentCategories as $parent)
                                    <option value="{{ $parent->id }}" {{ old('parent_id', $subcategory->parent_id) == $parent->id ? 'selected' : '' }}>{{ $parent->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control py-3" id="name" name="name" value="{{ old('name', $subcategory->name) }}" placeholder="e.g. National, International, Cricket" required autofocus>
                        </div>

                        <div class="mb-4">
                            <label for="slug" class="form-label fw-semibold text-secondary">Slug (SEO URL)</label>
                            <input type="text" class="form-control py-3" id="slug" name="slug" value="{{ old('slug', $subcategory->slug) }}" placeholder="auto-generated-slug" required>
                        </div>

                        <div class="mb-4">
                            <label for="description" class="form-label fw-semibold text-secondary">Description (Optional)</label>
                            <textarea class="form-control" id="description" name="description" rows="4" placeholder="Brief details about subcategory...">{{ old('description', $subcategory->description) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card bg-light bg-opacity-25 border-0 p-4 rounded-3 mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="file" class="form-control mb-2" id="image_upload" name="image_upload">
                                @if($subcategory->image)
                                    <div class="p-2 bg-white rounded border d-inline-block">
                                        <img src="{{ $subcategory->image }}" alt="Thumbnail" style="max-height: 80px;">
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" class="form-control py-3" id="sort_order" name="sort_order" value="{{ old('sort_order', $subcategory->sort_order) }}" required>
                            </div>
                            ...
                            <div class="form-check form-switch ps-0">
                                ...
                                <input class="form-check-input ms-0 border-secondary-subtle" style="width: 2.5em; height: 1.25em;" type="checkbox" id="status" name="status" value="1" {{ old('status', $subcategory->status ? '1' : '0') == '1' ? 'checked' : '' }}>
                            </div>
                        </div>
                    </div>
                </div>

...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-secondary mb-0">Modify login parameters or reset password.</p>
        </div>
                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control py-3" id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="e.g. John Doe" required autofocus>
                        </div>

                        <div class="mb-4">
                            <label for="email" class="form-label fw-semibold text-secondary">Email Address</label>
                            <input type="email" class="form-control py-3" id="email" name="email" value="{{ old('email', $user->email) }}" placeholder="e.g. john@newsportal.com" required>
                        </div>

                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                <label for="password" class="form-label fw-semibold text-secondary">Change Password (Optional)</label>
                                <input type="password" class="form-control py-3" id="password" name="password" placeholder="Leave blank to keep current">
                            </div>
                            <div class="col-md-6">
                                <label for="password_confirmation" class="form-label fw-semibold text-secondary">Confirm Change Password</label>
                                <input type="password" class="form-control py-3" id="password_confirmation" name="password_confirmation" placeholder="Re-type password">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card bg-light bg-opacity-25 border-0 p-4 rounded-3 mb-4">
                            ...
                            <div class="form-check form-switch ps-0">
                                ...
                                <input class="form-check-input ms-0 border-secondary-subtle" style="width: 2.5em; height: 1.25em;" type="checkbox" id="status" name="status" value="1" {{ old('status', $user->status ? '1' : '0') == '1' ? 'checked' : '' }} {{ $user->id === auth()->id() ? 'disabled' : '' }}>
                            </div>
                        </div>
                    </div>
                </div>

...
<div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark mb-1">Add Category</h2>
            <p class="text-secondary mb-0">Create a new parent category or subcategory.</p>
        </div>
        ...
        <div class="card-body p-5">
            <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="4" placeholder="Enter category details...">{{ old('description') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card bg-light bg-opacity-25 border-0 p-4 rounded-3 mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select py-3" id="layout_type" name="layout_type">
                                    <option value="standard" {{ old('layout_type') == 'standard' ? 'selected' : '' }}>Standard Layout</option>
                                    <option value="layout_1" {{ old('layout_type') == 'layout_1' ? 'selected' : '' }}>Layout 1 (Big Left, List Right)</option>
                                    <option value="layout_2" {{ old('layout_type') == 'layout_2' ? 'selected' : '' }}>Layout 2 (Middle Big, Sides Grid)</option>
                                    <option value="sports" {{ old('layout_type') == 'sports' ? 'selected' : '' }}>Sports Layout</option>
                                </select>
                            </div>

                            <div class="mb-4">
                                <label for="icon" class="form-label fw-semibold text-secondary">FontAwesome Icon Class</label>
                                <input type="text" class="form-control py-3" id="icon" name="icon" value="{{ old('icon') }}" placeholder="fa-solid fa-sports">
                            </div>

                            <div class="mb-4">
                                <label for="sort_order" class="form-label fw-semibold text-secondary">Sort Order</label>
                                <input type="number" class="form-control py-3" id="sort_order" name="sort_order" value="{{ old('sort_order', 0) }}" min="0">
                            </div>
                        ...
                        <div class="card bg-light bg-opacity-25 border-0 p-4 rounded-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control py-3" id="meta_title" name="meta_title" value="{{ old('meta_title') }}" placeholder="Title for Search Engines">
                            </div>

                            <div>
                                <label for="meta_description" class="form-label fw-semibold text-secondary">SEO Meta Description</label>
                                <textarea class="form-control" id="meta_description" name="meta_description" rows="3" placeholder="Snippet for search pages">{{ old('meta_description') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-top border-light mt-5 pt-4 text-end">
                    <button type="submit" class="btn btn-primary px-5 py-3 rounded-pill fw-semibold shadow-sm"><i class="fa-solid fa-circle-check me-2"></i> Save Category</button>
                </div>
            </form>
        </div>
    </div>
